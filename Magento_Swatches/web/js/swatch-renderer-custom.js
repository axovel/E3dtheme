    ...
    $.widget('mage.SwatchRenderer', {
        ...
        _RenderControls: function () {
            ...
            $.each(this.options.jsonConfig.attributes, function () {
                ...
                if ($widget.options.enableControlLabel) {
                    label +=
                        '<div onclick="swatchToggle('+item.id+')" class="' + classes.attributeLabelClass + '"><div class="swatch-label-wrap"></div><span class="swatch-label">' + item.label +
                        '</span><span class="asterik">*</span><i class="material-icons list-group-down arrow-down-icon">keyboard_arrow_down</i>' +
                        '<i class="material-icons list-group-down done-icon" style="display:none">done</i></div>'
                    '</span><span class="' + classes.attributeSelectedOptionLabelClass + '"></span>';
                }

                if ($widget.productForm) {
                    $('.product-options-bottom').before(input);
                ...
                container.append(
                    ...
                    label +
                    '<div style="display:none" class="' +'swatch-content-'+item.id +' '+ classes.attributeOptionsWrapper + ' clearfix">' +
                    options + select +
                    '</div>' + input +
                    '</div>'
                );
        ...
        _RenderSwatchOptions: function (config) {
            ...
            $.each(config.options, function () {
                ...
                if (type === 0) {
                    // Text
                    html += '<div onclick="swatchIcon('+this.id+')" class="' + optionClass + ' text" ' + attr + '>' + (value ? value : label) +
                        '</div>';
                } else if (type === 1) {
                    // Color
        ...
        _UpdatePrice: function () {
            ...
            $('.price-in.product-info-price').html($('.product-info-main .product-info-price').html());
...
function swatchToggle(id){
    requirejs(['jquery'],function($){
        $('.swatch-content-'+id).slideToggle(200);
    });
}